...
class InlineWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    // ---- Review: word-level diff (red = removed, green = added) ----
    box.addClass("sk-inline-review");
    const segs = wordDiff(this.s.original, this.s.suggestion);
    const changed = segs.some((s) => s.type !== "eq");

    if (!changed) {
      // Identical text — show it plainly so an empty diff never looks broken.
      box.createSpan({ cls: "sk-inline-nochange", text: "No changes suggested" });
    }
